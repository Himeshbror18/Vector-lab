...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const xTicks = Math.min(8, Math.max(4, Math.floor(maxX / 8)))
      for (let i = 0; i <= xTicks; i++) {
        const x = sx(i * maxX / xTicks)
        ctx.beginPath(); ctx.moveTo(x, pad.t); ctx.lineTo(x, groundY); ctx.stroke()
      }
      const yTicks = Math.min(6, Math.max(4, Math.floor(maxY / 4)))
      for (let i = 0; i <= yTicks; i++) {
        const y = sy(i * maxY / yTicks)
        ctx.beginPath(); ctx.moveTo(ox, y); ctx.lineTo(Math.min(w - pad.r, sx(maxX)), y); ctx.stroke()
      }
      ...
      ctx.fillText('height (m)', ox + 8, pad.t + 10)
